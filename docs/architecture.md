...
- **数据规模现实**：本项目实际接入殆知阁（约 20 亿字）等语料，去重后约 14.4 亿字符（token 化后约 7-9 亿 token），在此约束下设计模型规模
...
|--------|-------------|---------------|----------|
| 50M | ~1B token | 7-9 亿 token | 数据略偏少，需约 1.2 轮循环 |
| **~157M** | **~3.1B token** | **7-9 亿 token** | 需约 4 轮循环，可接受 |
| 300M | ~6B token | 7-9 亿 token | 需约 7-8 轮循环，略偏多 |

> 文言文的客观约束：本项目实际接入殆知阁（约 20 亿字）等语料，去重后约 14.4 亿字符（token 化后约 7-9 亿 token）。在设计上接受这个规模上限，而非为填补数据而去爬取版权不明的资源。
